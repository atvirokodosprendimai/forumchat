...
### Phase 6 — Comments — forum-style — status: completed

Goal: project page has a SSE-driven comment thread with edit-grace + delete.

1. [x] `repo` — `ListComments` (filters soft-deleted), `InsertComment`, `UpdateComment`, `SoftDeleteComment`, `CommentByID`
2. [x] `service` — `AddComment`, `UpdateComment` (enforces author+grace OR admin), `DeleteComment` (author OR admin, no grace check for author); markdown via `internal/render`
3. [x] `handler` — `PostComment` + `PostCommentEdit` + `PostCommentDelete`; signals bag extended with `projects_comment_body`/`projects_comment_edit`/`projects_comment_edit_id`
4. [x] Templ `ProjectCommentsFragment` — comment list with edit-in-place + delete buttons (visibility per `CanEdit`/`CanDelete`), textarea pinned at the bottom with Ctrl/Cmd+Enter handling via `evt.key === 'Enter' && !evt.shiftKey`
5. [x] SSE extension — `Event{Kind:"comments"}` triggers `pushComments`
6. [p] Scroll-to-bottom on new comment — deferred to Phase 7 polish. Comments naturally append; users already see the new row inline.
7. [x] main.go — 3 new routes + `cfg.EditGrace` plumbed into the projects service constructor
